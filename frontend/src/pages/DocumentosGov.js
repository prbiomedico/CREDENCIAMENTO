...
import { Upload, FileText, Download, Trash2, History, Pencil, Ban } from 'lucide-react';
...
const DocumentosGov = () => {
  ...
  const [editDialog, setEditDialog] = useState({ open: false, doc: null, nome: '', tipo: '', notes: '' });
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/atividades-raci`, { withCredentials: true })
      .then((res) => setAtividadesRaci(res.data.atividades || []))
      .catch(() => {});
  ...
  const abrirEdicao = (doc) => {
    setEditDialog({ open: true, doc, nome: doc.nome, tipo: doc.tipo, notes: doc.notes || '' });
  };
  ...
  const salvarEdicao = async (e) => {
    ...
    try {
      ...
      setEditDialog({ open: false, doc: null, nome: '', tipo: '', notes: '' });
      fetchDocumentos();
    } catch (error) {
  ...
  if (acessoNegado) {
    return (
      <DashboardLayout>
        <div className="p-6 lg:p-8" data-testid="documentos-gov-page">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setForm((prev) => ({
                        ...
                        categoria: value,
                        tipo: '',
                        estado_sigla: (value === 'estado_detran' && isDetranAdmin && !isAdmin) ? (user?.detran_uf || '') : '',
                        atividade_raci_numero: '',
                      }))}
                    >
