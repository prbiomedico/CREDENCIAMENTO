...
const STATUS_CFG = {
  pendente:     { label: "Pendente",     color: "hsl(var(--sigcr-warning))", bg: "rgba(255,183,77,0.12)" },
      ...
      <div style={{ background:"hsl(var(--background))", border:"1.5px solid rgba(33,150,243,0.25)", borderRadius:20, padding:32, width:460 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {[
            ...
            <div key={f.key}>
              <label style={{ fontSize:11, fontWeight:700, color:"hsl(var(--muted-foreground))", textTransform:"uppercase", letterSpacing:"0.08em", display:"block", marginBottom:5 }}>{f.label}</label>
              <input value={form[f.key]} onChange={e => setForm({...form,[f.key]:e.target.value})} placeholder={f.ph}
                style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:"1.5px solid rgba(255,255,255,0.1)", borderRadius:9, color:"hsl(var(--foreground))", padding:"9px 12px", fontSize:13, outline:"none", boxSizing:"border-box" }} />
            </div>
          ))}
        </div>
        <div style={{ display:"flex", gap:10, marginTop:24 }}>
          <button onClick={onClose} style={{ flex:1, padding:"11px", borderRadius:9, cursor:"pointer", background:"transparent", border:"1.5px solid rgba(255,255,255,0.1)", color:"hsl(var(--muted-foreground))", fontSize:13, fontWeight:700 }}>Cancelar</button>
          <button onClick={handleSubmit} disabled={loading} style={{ flex:2, padding:"11px", borderRadius:9, cursor:"pointer", background:"linear-gradient(135deg,hsl(var(--sigcr-accent)),hsl(var(--sigcr-accent)))", border:"none", color:"#fff", fontSize:13, fontWeight:800 }}>
      ...
      <div style={{ background:"hsl(var(--background))", border:"1.5px solid rgba(33,150,243,0.25)", borderRadius:20, padding:32, width:500, maxHeight:"90vh", overflowY:"auto" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
          <div>
            ...
            <div style={{ fontSize:12, color:"hsl(var(--muted-foreground))", marginTop:2 }}>{esteira.registradora}</div>
          </div>
          <button onClick={onClose} style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:8, color:"hsl(var(--muted-foreground))", padding:"6px 12px", cursor:"pointer", fontSize:16 }}>x</button>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {[
            ...
            <div key={f.key}>
              ...
              {f.type === "select" ? (
                <select value={form[f.key]} onChange={e => setForm({...form,[f.key]:e.target.value})}
                  style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:"1.5px solid rgba(255,255,255,0.1)", borderRadius:9, color:"hsl(var(--foreground))", padding:"9px 12px", fontSize:13, outline:"none", appearance:"none" }}>
                  <option value="">Selecione...</option>
                  {f.opts.map(o => <option key={o.v} value={o.v} style={{ background:"hsl(var(--background))" }}>{o.l}</option>)}
                </select>
              ) : f.type === "textarea" ? (
                <textarea value={form[f.key]} onChange={e => setForm({...form,[f.key]:e.target.value})} placeholder={f.ph} rows={3}
                  style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:"1.5px solid rgba(255,255,255,0.1)", borderRadius:9, color:"hsl(var(--foreground))", padding:"9px 12px", fontSize:13, outline:"none", resize:"vertical", fontFamily:"inherit", boxSizing:"border-box" }} />
              ) : (
                <input type={f.type} value={form[f.key]} onChange={e => setForm({...form,[f.key]:e.target.value})} placeholder={f.ph}
                  style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:"1.5px solid rgba(255,255,255,0.1)", borderRadius:9, color:"hsl(var(--foreground))", padding:"9px 12px", fontSize:13, outline:"none", boxSizing:"border-box", colorScheme:"dark" }} />
              )}
            </div>
          ))}
        </div>
...
export default function Esteiras() {
  ...
  return (
    <div style={{ display:"flex", height:"100%", minHeight:"100vh", background:"hsl(var(--background))", fontFamily:"system-ui, sans-serif", color:"hsl(var(--foreground))" }}>
      <div style={{ width:320, borderRight:"1px solid rgba(255,255,255,0.06)", padding:"24px 18px", overflowY:"auto", flexShrink:0 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
          <span style={{ fontWeight:800, fontSize:15, color:"hsl(var(--muted))" }}>Esteiras ({esteiras.length})</span>
          <button onClick={() => setModal("nova")} style={{ fontSize:11, fontWeight:800, padding:"4px 12px", borderRadius:20, background:"linear-gradient(135deg,hsl(var(--sigcr-accent)),hsl(var(--sigcr-accent)))", border:"none", color:"#fff", cursor:"pointer" }}>+ Nova</button>
        </div>
        <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar registradora..."
          style={{ background:"rgba(255,255,255,0.04)", border:"1.5px solid rgba(255,255,255,0.08)", borderRadius:9, color:"hsl(var(--foreground))", padding:"9px 13px", fontSize:12, outline:"none", width:"100%", marginBottom:14, boxSizing:"border-box", fontFamily:"inherit" }} />
        {loading ? <div style={{ color:"hsl(var(--muted-foreground))", textAlign:"center", padding:"40px 0" }}>Carregando...</div>
        : erro ? <div style={{ color:"hsl(var(--sigcr-danger))", fontSize:12, textAlign:"center", padding:"20px 0" }}>{erro}</div>
        : filtradas.length === 0 ? (
          <div style={{ color:"hsl(var(--muted-foreground))", fontSize:13, textAlign:"center", padding:"40px 0" }}>
            Nenhuma esteira.<br/>
            <button onClick={() => setModal("nova")} style={{ marginTop:10, fontSize:12, color:"hsl(var(--sigcr-accent))", background:"none", border:"none", cursor:"pointer", textDecoration:"underline" }}>Criar primeira</button>
          </div>
        ) : filtradas.map(est => {
            ...
            <div key={est.esteira_id} style={cardStyle(sel)} onClick={() => setSelectedId(est.esteira_id)}>
              ...
              <div style={{ marginBottom:8 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  <span style={{ fontSize:11, color:"hsl(var(--muted-foreground))" }}>Progresso</span>
                  <span style={{ fontSize:11, fontWeight:700, color: p===100 ? "hsl(var(--sigcr-success))" : "hsl(var(--sigcr-warning))" }}>{p}%</span>
                </div>
                <div style={{ height:3, borderRadius:3, background:"rgba(255,255,255,0.06)" }}>
                  <div style={{ height:"100%", borderRadius:3, background: p===100 ? "linear-gradient(90deg,hsl(var(--sigcr-success)),hsl(var(--sigcr-success)))" : "linear-gradient(90deg,hsl(var(--sigcr-warning)),hsl(var(--sigcr-warning)))", width:`${p}%`, transition:"width 0.4s" }} />
                </div>
              </div>
              <div style={{ fontSize:11, color:"hsl(var(--muted-foreground))" }}> {est.responsavel}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"32px 36px" }}>
        {loading ? <div style={{ color:"hsl(var(--muted-foreground))", textAlign:"center", padding:"80px 0" }}>Carregando...</div>
        : !selected ? <div style={{ color:"hsl(var(--muted-foreground))", textAlign:"center", padding:"80px 0" }}>{esteiras.length === 0 ? "Nenhuma esteira ainda." : "Selecione uma esteira."}</div>
        : (
          <>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28 }}>
              <div>
                <div style={{ fontWeight:800, fontSize:22, color:"hsl(var(--muted))", marginBottom:6 }}>{selected.registradora}</div>
                <div style={{ display:"flex", gap:14, fontSize:12, color:"hsl(var(--muted-foreground))", flexWrap:"wrap" }}>
                  <span> {selected.detran}</span>
                  <span> {selected.cnpj}</span>
                  <span> {selected.responsavel}</span>
                </div>
              </div>
              <button onClick={() => setModal("evento")} style={{ background:"linear-gradient(135deg,hsl(var(--sigcr-accent)),hsl(var(--sigcr-accent)))", border:"none", borderRadius:11, color:"#fff", padding:"11px 22px", fontWeight:800, fontSize:13, cursor:"pointer", flexShrink:0 }}>
                + Criar Evento
              </button>
            </div>
            ...
            <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:14, padding:"18px 22px", marginBottom:28 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                <span style={{ fontWeight:700, fontSize:13, color:"hsl(var(--muted-foreground))" }}>Progresso da Esteira</span>
                <span style={{ fontWeight:800, fontSize:20, color: prog(selected.eventos)===100 ? "hsl(var(--sigcr-success))" : "hsl(var(--sigcr-warning))" }}>{prog(selected.eventos)}%</span>
              </div>
              <div style={{ height:5, borderRadius:5, background:"rgba(255,255,255,0.06)" }}>
                <div style={{ height:"100%", borderRadius:5, background: prog(selected.eventos)===100 ? "linear-gradient(90deg,hsl(var(--sigcr-success)),hsl(var(--sigcr-success)))" : "linear-gradient(90deg,hsl(var(--sigcr-warning)),hsl(var(--sigcr-warning)))", width:`${prog(selected.eventos)}%`, transition:"width 0.5s" }} />
              </div>
            ...
            <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:14, padding:"24px 26px" }}>
              <div style={{ fontWeight:800, fontSize:13, color:"hsl(var(--muted-foreground))", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:22 }}>Fluxo da Esteira</div>
                ...
                return (
                  <div key={etapa.id} style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"center" }}>
                      ...
                      {!isLast && <div style={{ width:2, minHeight:28, background: ev.status==="concluido" ? "hsl(var(--sigcr-success))" : "rgba(255,255,255,0.07)", margin:"3px 0" }} />}
                    </div>
                    <div style={{ paddingBottom: isLast ? 0 : 18, flex:1 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                        <span style={{ fontWeight:700, fontSize:13, color:"hsl(var(--foreground))" }}>{etapa.nome}</span>
                      ...
                      {ev.data && <div style={{ fontSize:11, color:"hsl(var(--muted-foreground))", marginBottom:2 }}>{new Date(ev.data).toLocaleDateString("pt-BR")}</div>}
                      {ev.obs && <div style={{ fontSize:12, color:"hsl(var(--muted-foreground))", fontStyle:"italic" }}>{ev.obs}</div>}
                    </div>
                  </div>
                );
              })}
