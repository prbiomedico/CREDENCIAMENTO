        ...
        <section className="space-y-4" aria-label="Processos e pendências por DETRAN">
          ...
          <div className="overflow-hidden rounded-lg border border-border bg-card">
            ...
            {Array.from(new Set(linhas.map(p => p.uf))).map(uf => <section key={uf} aria-label={`Processos DETRAN-${uf}`}>
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/20 px-4 py-2"><h2 className="text-sm font-semibold">DETRAN-{uf}</h2><Button asChild size="sm" variant="outline"><Link to={`/acompanhamento/${uf}?empresa=${encodeURIComponent(companyId)}`}>Abrir acompanhamento completo</Link></Button></div>
