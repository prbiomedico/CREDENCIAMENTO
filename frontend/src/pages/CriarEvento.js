...
const CORES = {
  credenciamento: { ativo: 'bg-primary-500/20 border-primary-500/50', inativo: 'bg-zinc-900/50 border-zinc-800', badge: 'bg-primary-500/10 text-primary-400' },
  ...
  chamamento: { ativo: 'bg-secondary-500/20 border-secondary-500/50', inativo: 'bg-zinc-900/50 border-zinc-800', badge: 'bg-secondary-500/10 text-secondary-400' },
};
      ...
      <div className="p-6 lg:p-8 max-w-4xl mx-auto">
        ...
        {step === 1 && tmpl && (
          <div className="space-y-5">
            ...
            <div><Label className="text-zinc-300 text-sm mb-1.5 block">Ttulo *</Label><Input value={form.titulo} onChange={e => setForm(f => ({...f, titulo: e.target.value}))} className="bg-zinc-950 border-zinc-800 focus:border-primary-500 text-white" placeholder="Ex: Credenciamento de Registradoras DF 2026" /></div>
            ...
            <div className="flex gap-3"><Button variant="outline" onClick={() => setStep(0)} className="border-zinc-700 text-zinc-300">Voltar</Button><Button onClick={() => setStep(2)} disabled={!form.titulo || !form.uf} className="bg-primary-500 hover:bg-primary-600 text-white">Prximo <ChevronRight className="h-4 w-4 ml-1" /></Button></div>
          </div>
        )}
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {(Array.isArray(tmpl?.documentos_padrao) ? tmpl.documentos_padrao : []).map(doc => {
                ...
                return <button key={doc} onClick={() => toggleDoc(doc)} className={`flex items-center gap-2 p-3 rounded-xl border text-xs text-left transition-all font-mono ${on ? 'bg-primary-500/15 border-primary-500/40 text-primary-300' : 'bg-zinc-900/50 border-zinc-800 text-zinc-400 hover:border-zinc-700'}`}><div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${on ? 'bg-primary-500 border-primary-500' : 'border-zinc-600'}`}>{on && <Check className="h-3 w-3 text-white" />}</div>{doc}</button>;
              })}
            </div>
            <div className="bg-zinc-900/30 rounded-lg p-3 border border-zinc-800"><p className="text-xs text-zinc-500 mb-2">{form.documentos_obrigatorios.length} selecionados</p><div className="flex flex-wrap gap-1">{form.documentos_obrigatorios.map(d => <span key={d} className="text-[10px] bg-primary-500/10 text-primary-300 px-2 py-0.5 rounded-full font-mono flex items-center gap-1">{d}<button onClick={() => toggleDoc(d)}></button></span>)}</div></div>
            <div className="flex gap-3"><Button variant="outline" onClick={() => setStep(1)} className="border-zinc-700 text-zinc-300">Voltar</Button><Button onClick={() => setStep(3)} className="bg-primary-500 hover:bg-primary-600 text-white">Prximo <ChevronRight className="h-4 w-4 ml-1" /></Button></div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {form.timeline.map((item, i) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                  <div className="w-6 h-6 rounded-full bg-primary-500/20 border border-primary-500/30 flex items-center justify-center shrink-0"><span className="text-[10px] font-mono text-primary-400">{i+1}</span></div>
            ...
            <div className="flex gap-3"><Button variant="outline" onClick={() => setStep(2)} className="border-zinc-700 text-zinc-300">Voltar</Button><Button onClick={() => setStep(4)} className="bg-primary-500 hover:bg-primary-600 text-white">Revisar <ChevronRight className="h-4 w-4 ml-1" /></Button></div>
          </div>
        )}
          ...
          <div className="space-y-5">
            <Card className="bg-zinc-900/50 border-zinc-800">
              ...
              <CardContent className="space-y-4">
                ...
                <div><p className="text-xs text-zinc-500 mb-2 font-mono uppercase">Timeline ({form.timeline.length} etapas)</p><div className="space-y-1">{form.timeline.map((t, i) => <div key={i} className="flex items-center gap-2 text-xs"><div className="w-1.5 h-1.5 rounded-full bg-primary-500 shrink-0" /><span className="text-zinc-300">{t.etapa}</span><span className="text-zinc-600 ml-auto font-mono">+{t.dias_corridos}d</span></div>)}</div></div>
              </CardContent>
            </Card>
        ...
        {step === 5 && eventoSalvo && (
          <div className="text-center space-y-6">
            ...
            <div className="flex gap-3 justify-center">
              ...
              <Button onClick={() => navigate('/editais')} className="bg-primary-500 hover:bg-primary-600 text-white">Ver Eventos</Button>
            </div>
          </div>
        )}
      </div>
