...
Deploy: commit `a83db9b` (auto-commit pre-deploy, escopo `frontend/` apenas), push confirmado pro GitHub antes do build. Release `releases/20260827-menubar-overflow-button-refino` (hash `main.a029c446.js` / `main.847faf19.css`), swap atômico do symlink `current`. Rollback: release anterior `20260827-1437-dashboard-menubar-clickfix`. Health check pós-deploy: site `sigcr.com.br` 200 servindo o hash novo; backend `api.sigcr.com.br/api/stats` 401 sem token (backend não foi tocado, seguiu 401 como antes). Backend não foi tocado (mudança 100% frontend) — o lote pendente de ~490 linhas em `backend/server.py` e `README.md` permanecem intocados e não commitados, isolados do escopo deste deploy pelo `git-sync-or-die.sh frontend`.

## 50. Fusão "Transparência" (Editais + Portarias) — fatia 1: unificação de Editais — ✅ DEPLOYADO (2026-08-27)

Decisão do Pedro após o levantamento anterior (ver `project-sigcr-dashboard-menubar-20260827` e o dossiê publicado): implementar a fusão em 3 fatias. Esta é a primeira — unificar consulta e gestão de editais numa tela só, replicando o padrão contextual que `Portarias.js` já usa (`podeGerenciar`).

**Mudanças**: `Editais.js` ganhou o dialog completo de criar/editar edital (título/UF/status/data de encerramento/descrição/documentos obrigatórios/upload de anexos/upload de termo de adesão) — todo o conteúdo que vivia em `GestaoEditais.js`, agora atrás de `podeGerenciar = ['sigcr_admin','detran','detran_admin'].includes(user?.perfil)`, mesmo array que `Portarias.js` já usa. Botão "Editar" (ícone lápis) aparece por card só pra quem gerencia; botão "Novo Edital" só aparece no cabeçalho pra quem gerencia. Decisão de UX tomada nesta fatia (não pedida explicitamente, mas consequência natural de unificar): "Candidatar-se" só aparece pra quem NÃO gerencia — não fazia sentido DETRAN candidatar-se ao próprio edital.

`GestaoEditais.js` removido (arquivo, rota `/gestao-editais` em `App.js`, entradas de menu em `navMenus.js` — estavam em `NAV_DETRAN` e `NAV_ADMIN_EXTRA`). Backend não foi tocado — mesmo `GET/POST/PATCH /editais` de sempre, nenhum contrato mudou.

**Teste isolado**: worktree detached de HEAD + patch da fatia aplicado, build real, servido localmente, Playwright com bypass de auth via `localStorage.__sigcr_preview_user__` (patch aplicado só no worktree de teste, nunca no repo). Confirmado via contagem de elementos (não só visual): perfil registradora — zero botão "Novo Edital", zero "Editar", exatamente 1 "Candidatar-se" (só no edital com status "aberto"), fluxo de candidatura completo funcionando (toast de sucesso). Perfil DETRAN — 1 "Novo Edital", zero "Candidatar-se", 2 "Editar" (uma por card); criação de edital via dialog completo (POST /editais chamado, confirmado por mock de rede) e edição de edital existente pré-preenchendo todos os campos incl. anexo e documento obrigatório já cadastrados (PATCH /editais/{id} chamado). Nenhuma capacidade de `GestaoEditais.js` ficou pra trás.

Deploy: commit `25a6401` (auto-commit + push, escopo `frontend/`), release `releases/20260827-fatia1-unificacao-editais` (hash `main.21ab5e20.js`), rollback = release anterior `20260827-menubar-overflow-button-refino`. Health check: site 200, hash novo confirmado, `GET /api/editais` segue 401 sem token (backend intocado). Nota: o push desta fatia teve um erro transitório de "cannot lock ref" do GitHub na primeira tentativa (o commit já tinha ido, só o retorno do comando falhou) — reexecutar o deploy resolveu sem re-commitar nada.

## 51. Fusão "Transparência" — fatia 2: acesso da financeira a Portarias e Editais — ✅ DEPLOYADO (2026-08-27)

Decisão do Pedro: financeira ganha acesso consulta a Portarias e Editais — gap pré-existente (bloqueada em ambas as rotas desde sempre, sem relação com esta fusão) que ele decidiu fechar agora.

**Investigação antes de implementar**: o backend já estava pronto para financeira nas duas rotas — `GET /portarias` e `GET /editais` não têm `require_perfil` (só exigem usuário autenticado), e o filtro de rascunho em `GET /portarias` já tratava `("registradora", "financeira")` como o mesmo grupo desde a correção de 2026-08-12 (import de portarias históricas). `POST /solicitacoes` (candidatura) usa `require_acesso_total`, que checa contrato assinado/assinatura ativa — não perfil — então financeira já podia candidatar-se tecnicamente, só não alcançava a tela. Conclusão: **mudança 100% de frontend**, nada a ajustar no backend.

**Mudanças**: `App.js` — `financeira` adicionada ao `perfilPermitido` de `/portarias` e `/editais`. `navMenus.js` — `NAV_FINANCEIRA` ganhou os itens `Editais Abertos` e `Portarias` (sem `destaque`, pra não inflar o Menubar do Dashboard de novo — ver item 49 — já que a fatia 3 vai colapsar tudo num item só de qualquer forma).

**Teste isolado**: mesmo padrão de worktree + Playwright das fatias anteriores, perfil `financeira` simulado. Confirmado: `/editais` renderiza normalmente pra financeira (edital visível, "Candidatar-se" disponível, zero botão de gestão — paridade exata com registradora); `/portarias` idem (lista agrupada por UF visível, zero "Editar"/"Criar Evento" — `podeGerenciar` continua `false` pra financeira, ela ganhou consulta, não gestão); sidebar mostra os 2 itens novos.

Deploy: release `releases/20260827-fatia2-financeira-acesso` (hash `main.0a74f0b5.js`), rollback = release anterior `20260827-fatia1-unificacao-editais`. Backend não foi tocado. Health check: site 200, hash novo confirmado.

## 52. Fusão "Transparência" — fatia 3: um único ambiente/rota/menu — ✅ DEPLOYADO (2026-08-27)

Última fatia da fusão pedida pelo Pedro. `Portarias.js` e `Editais.js` (já unificado consulta+gestão na fatia 1) foram substituídos por um único componente, `AreaTransparencia.js`, com abas "Portarias"/"Editais" (`Tabs` shadcn/Radix, já disponível no projeto, sem uso até então).

**Decisão de nomenclatura confirmada com o Pedro**: o rótulo "Transparência" é reaproveitado no menu logado — sem colisão real de rota, só de nome, exatamente como ele avaliou. A página pública sem login (`/transparencia/:uf`, consumindo `/api/public/editais/{uf}`) continua existindo intacta, em rota separada. O ambiente logado usa a rota já estabelecida `/portarias` (não `/transparencia`) — reaproveita a URL que já estava com `RotaProtegida` liberada pros 4 perfis desde a fatia 2, evitando introduzir uma rota nova só pra isso. Confirmado registrando as duas rotas lado a lado: `/transparencia` → página pública (`Transparencia.js`, inalterado), `/portarias` → ambiente logado (`AreaTransparencia.js`, novo) — zero sobreposição de path.

**Mudanças**: `App.js` — import de `Portarias`/`Editais` trocado por `AreaTransparencia`; rota `/portarias` aponta pro componente novo; rota `/editais` virou um redirect (`<Navigate to="/portarias?aba=editais" replace />`) pra não quebrar bookmarks antigos. `navMenus.js` — os 2 itens separados (`Editais`/`Editais Abertos` + `Portarias`) viraram 1 item só, rótulo "Transparência", em `NAV_REGISTRADORA`, `NAV_DETRAN` e `NAV_FINANCEIRA`. A aba ativa é estado de URL (`?aba=editais`, default "portarias") — preserva deep-link (o card "Portarias" do Dashboard, que já usa `?status=vigente`, continua funcionando sem mudança).

**O que não mudou** (por decisão explícita do escopo, ver achado abaixo): as duas coleções Mongo (`db.portarias`, `db.editais`) continuam separadas — fusão 100% de UI/navegação. Nenhum contrato de rota do backend mudou.

**Teste isolado**: worktree detached de HEAD + patch acumulado das 3 fatias, build real, Playwright com o mesmo bypass de auth. Confirmado por perfil: registradora — 1 só "Transparência" no menu (zero "Editais"/"Portarias" soltos), abas trocam de conteúdo sem reload (client-side, via `useSearchParams`), zero botão de gestão em nenhuma aba; DETRAN — "Criar Evento" contextual na aba Portarias, "Novo Edital" contextual na aba Editais, "Editar" em ambas depois de expandir/localizar o item; financeira — item único "Transparência" aparece no menu (herda o acesso da fatia 2); redirect de `/editais` pra `/portarias?aba=editais` confirmado (URL final + conteúdo da aba Editais carregado corretamente).
...
## 54. Fix de permissão "Trocar Visão": nenhuma ação de gestão/admin some quando sigcr_admin simula empresa — ✅ DEPLOYADO (2026-08-29)

Pedro identificou (via `AreaTransparencia.js`, que já vinha do fix da fatia 3 da fusão Transparência) que a simulação "ver como" (`viewingAs`, ver `project-sigcr-verascomo-fix-20260813`) só afetava o que o *backend* devolvia (via `view_as_company_id`/`view_as_detran_uf` injetado pelo interceptor axios) — nenhuma das 5 telas de gestão consultava `useViewContext` pra decidir o que *mostrar*. Resultado: sigcr_admin "vendo como" uma Registradora/Financeira continuava vendo (e podendo clicar) botões de ação de admin que uma empresa de verdade nunca teria, quebrando a premissa da simulação ("ver exatamente como a empresa vê").

**Achado mais grave que o esperado em `PainelConferencia.js`**: os botões Conforme/Inconforme/Homologar não tinham NENHUM gate de perfil — só checavam `item.status === 'enviado'`. Qualquer usuário que chegasse nessa tela (rota já protegida por `RotaProtegida` pra sigcr_admin/detran/detran_admin, então não explorável por um usuário real de fora, mas exposta por completo durante a simulação) via os botões de conferência/homologação sempre visíveis. Corrigido com uma variável nova, `podeConferir`.

**Padrão aplicado nas 5 telas** (`AreaTransparencia.js`, `Registradoras.js`, `Financeiras.js`, `PainelConferencia.js`, `Dashboard.js`): `const { viewingAs } = useViewContext();` + `&& viewingAs?.tipo !== 'empresa'` em toda variável de gate que hoje só olha `user.perfil` (`podeGerenciar`, `ehAdmin`, `isDetranOuAdmin`) — deliberadamente só pra `tipo === 'empresa'` (Registradora/Financeira), não pra `tipo === 'detran'` (simular um DETRAN de UF específica continua com as ações de DETRAN, que é o comportamento correto — só empresa nunca gerencia).

**Teste isolado**: worktree + Playwright (técnica documentada em `project-sigcr-empresa-split-portarias-uf`), bypass de Keycloak via `localStorage.__sigcr_preview_user__`, dirigindo a UI real de "Trocar Visão" (não atalho interno) — inclusive descoberto e contornado no processo que `DashboardLayout` renderiza a sidebar 2x (desktop+mobile, só `lg:hidden` via CSS) e que trocar de badge sempre navega pra `/dashboard` (`handleTrocarPerfil`), então cada cenário precisou voltar pra rota-alvo via clique de link real (nunca `page.goto`, que reseta o contexto React). 33 asserções: perfil real de cada tipo sem simulação (base), sigcr_admin simulando Registradora, sigcr_admin simulando Financeira, e sigcr_admin simulando DETRAN-SP (controle negativo — confirma que o fix não mexeu no que está fora do escopo pedido). 33/33 passaram.

**Deploy**: frontend only (fix é 100% de exibição condicional, nenhuma rota/contrato de API mudou). Commit `7d80a14` (auto-commit + push, escopo `frontend/`), release `releases/20260828-permissao-viewcontext-5telas`, rollback = release anterior `20260827-ciclo-notificacao-portaria-submissao` (`ln -s releases/20260827-ciclo-notificacao-portaria-submissao current.tmp && mv -T current.tmp current`, rodar de `/opt/sigcr/frontend`). Site 200 pós-deploy.

## 55. Catálogo de checklist de portaria: substituído o seed reduzido (12 itens genéricos) pelos dois catálogos oficiais completos — ✅ DEPLOYADO (2026-08-29)

Pedido do Pedro: o catálogo reutilizável usado pelo wizard Criar Evento (passo Documentos) e pela edição de checklist em `AreaTransparencia.js` tinha só 12 itens genéricos, todos `perfil_alvo=registradora` — nada pra financeira, e sem relação com nenhum texto regulatório real. Trocado pelos dois catálogos oficiais já usados em produção pelo checklist de cadastro-base/upload de documentos, reaproveitados direto de `server.py` (`CHECKLIST_CONTRAN_807`/`CHECKLIST_DETRAN_DF_003_2022`), sem retranscrever texto: Resolução CONTRAN 807 completa (31 itens, 6 blocos) → `perfil_alvo=registradora`; Edital DETRAN-DF nº 003/2022 completo (12 itens, 4 blocos) → `perfil_alvo=financeira`.

**Encontrado já em progresso no working tree ao retomar a sessão** (server.py, migração, `ChecklistCatalogoPicker.js` reescrito, README.md novo — tudo não commitado, sem teste isolado nem deploy ainda registrado). Sob autonomia total, completei o que faltava: teste isolado, migração em produção e os 2 deploys.

**Mudanças**: `server.py` — `BLOCO_PORTARIA_NOMES_POR_PERFIL` (cada perfil tem sua própria numeração/nomenclatura de bloco, não comparável entre si — 1-6 pra registradora, 1-4 pra financeira; a validação de bloco em `POST`/`PATCH /checklist-catalogo` agora é sempre condicionada ao `perfil_alvo` do item, inclusive quando o PATCH troca perfil e bloco em requisições separadas). `ChecklistCatalogoPicker.js` reescrito pra agrupar primeiro por perfil (badge "Registradora"/CONTRAN 807 azul, "Financeira"/Edital DETRAN-DF emerald) e dentro de cada um por bloco; form "+ criar item novo" troca a lista de blocos disponíveis dinamicamente ao trocar o perfil selecionado. Migração idempotente (`migrations/2026_08_29_checklist_catalogo_portaria_completo.py`): insere os 43 itens novos (no-op se já existem), desativa (nunca apaga) os 12 itens antigos do catálogo reduzido — checado individualmente contra `db.portarias.checklist_itens[].catalogo_item_id` antes de desativar (nenhum estava em uso por portaria real). Snapshots de portarias já publicadas (`PortariaChecklistItem`) não são tocados — são cópia independente do catálogo no momento da criação/edição, então a troca de catálogo não altera retroativamente nada já publicado. Corrigidos de passagem 2 comentários que tinham ficado desatualizados (mencionavam "12 itens"/"só registradora").

**Teste isolado — backend**: `test_checklist_catalogo.py` (já preparado no working tree), rodado contra banco descartável `sigcr_devtest_checklistcatalogo` no mongod de devtest via `httpx.AsyncClient(transport=ASGITransport(app=srv.app))`. 15/15 checks: seed popula 43 itens (31+12) com os `item_id` batendo exatamente com `CHECKLIST_CONTRAN_807`/`CHECKLIST_DETRAN_DF_003_2022` (mesma fonte, sem retranscrição), seed é idempotente, `GET /checklist-catalogo` (com e sem filtro `perfil_alvo`) retorna as contagens certas, `POST`/`PATCH` aceitam bloco válido pro perfil e rejeitam (400) bloco inválido pro perfil — inclusive o caso de trocar perfil e bloco em requisições separadas mantendo a validação coerente. Migração também validada em 2 rodadas: `--dry-run` e real contra um banco devtest com estado replicado do catálogo antigo real (5 itens de amostra + 1 portaria fake referenciando um deles) — confirmado que o item em uso é sinalizado `[EM USO]` no log mas desativado mesmo assim, sem afetar o snapshot da portaria, e que rodar a migração 2x é no-op na segunda vez.

**Teste isolado — frontend**: build real em worktree (`main.5fc2eea7.js`, sem warnings no arquivo alterado) servido localmente com fallback de SPA, Playwright com bypass `localStorage.__sigcr_preview_user__` dirigindo o wizard Criar Evento de verdade (perfil `detran_admin`) até o passo Documentos. 10/10 checks: as 2 seções (badges + fonte regulatória) visíveis, 43 checkboxes renderizados, nomes de bloco corretos e não comparáveis entre perfis (bloco 6 registradora = "Declaratórias Gerais", bloco 4 financeira = "Declaratórias"), seleção de 1 item de cada perfil reflete no contador, e o form "+ criar item novo" troca corretamente de 6 pra 4 opções de bloco ao trocar o perfil pra financeira. Screenshot conferido visualmente.

**Deploy**: backend — commit `567e9db` (auto-commit + push, escopo `backend/`, inclui a migração), rollback `sigcr-backend:pre-deploy-rollback-20260829-0940`. Migração rodada em produção logo em seguida (`docker exec sigcr-backend`): 43 itens novos inseridos, 13 itens antigos desativados (nenhum em uso — confirmado antes de rodar), idempotência reconfirmada rodando 2x. Health check: `GET /api/checklist-catalogo` e `GET /api/portarias` 401 (rotas vivas, exigem auth), sem erro/traceback nos logs do container. Frontend — commit `a2d7da9`, release `releases/20260829-checklist-catalogo-completo` (hash `main.5fc2eea7.js` — mesmo hash do build já testado com Playwright, sem divergência entre o que foi testado e o que foi ao ar), rollback = release anterior `20260828-permissao-viewcontext-5telas`. Site 200, `/criar-evento` e `/portarias` 200 pós-deploy. README.md commitado à parte (`b59e6ac`, fora do escopo dos scripts de deploy) documentando os padrões de deploy — conteúdo já existia no working tree, só formalizado em git.

**Importante — item 55 deliberadamente NÃO aplicou o catálogo novo a nenhuma portaria existente** (só trocou o catálogo reutilizável). O backfill nas portarias reais das 27 UFs é o item 56, abaixo.

## 56. Backfill do checklist novo (43 itens, item 55) nas 26 portarias vigentes reais + dedup RN/SP — ✅ DEPLOYADO (2026-08-29)

Pedido do Pedro, mesma sessão do item 55: aplicar `checklist_itens` (catálogo de 43 itens) na portaria vigente de cada uma das 27 UFs, com decisões fechadas por ele:
- **12 UFs sem ambiguidade** (AC, AM, AP, ES, GO, MG, MS, PB, PI, RJ, RR, TO): única portaria existente, aplicado direto.
- **11 UFs resolvidas por "mais recente" por número/ano no título** (CE→172-2026, MT→162-2026, RS→028-2026, SC→09-2026, DF→Instrução 199-2026, RO→1098-2025, AL→2738-2024, MA→Edital 01-2026, PE→Edital 15-2026, PR→409-2023, BA→169-2024).
- **SE→310-2025** (portaria de SUSPENSÃO de novos credenciamentos) — decisão explícita do Pedro de seguir a regra padrão mesmo sendo suspensão; ele decide depois com o DETRAN-SE se isso deve bloquear inscrição de fato (hoje não bloqueia — testado, ver abaixo).
- **PA — FORA do backfill, pendência real**: as 2 únicas portarias de PA (`2930-2012 - Registro de Contrato`, `Taxas/Extrato Termo Aditivo`) não são atos de credenciamento. **DETRAN-PA parece não ter nenhum ato de credenciamento publicado no sistema** — precisa ser resolvido com o DETRAN-PA (importar o ato real, se existir) antes de PA poder receber checklist/aceitar submissões.
- **RN**: confirmado via `pdftotext` no PDF real (`port_f80832657d62`, Diário Oficial do RN, 5 páginas) que o ato é a "PORTARIA 651/2026 -GADIR" — idêntico à duplicata importada `port_ea5e8364a2a0` ("PORTARIA 651-2026"). Duplicata removida (soft-delete); as outras 2 portarias de RN (308-2021, Edital 06-2026) são atos genuinamente diferentes, mantidas intactas.
- **SP**: confirmado via `pdftotext` no PDF real (`port_08af5c3feeaf`) que é o "EDITAL DE CREDENCIAMENTO Nº 17, DE 27 DE ABRIL DE 2026" — idêntico à duplicata importada `port_969354f40e27`. Duplicata removida (soft-delete).

**Migração** (`migrations/2026_08_29_backfill_checklist_portarias_vigentes.py`): idempotente — cada portaria recebe um snapshot novo de `PortariaChecklistItem` (43 itens, `catalogo_item_id` apontando pro catálogo pra rastreabilidade, mas o snapshot não muda retroativamente se o catálogo for editado depois), pulada se já tiver algum item com `catalogo_item_id` do catálogo ativo atual. Soft-deletes de RN/SP checam `db.submissoes` antes de remover (nenhuma existia). Testada em 3 rodadas contra devtest com todas as 60 portarias reais copiadas (`--dry-run`, real, 2ª rodada real pra confirmar no-op), depois em produção do mesmo jeito (`--dry-run` bateu exatamente com o plano, real aplicou, 2ª rodada confirmou idempotência).

**Teste E2E `POST /submissoes`** (devtest, banco com os 60 documentos reais + backfill real aplicado, empresas/sessões sintéticas via fallback `session_token` legado, `httpx.AsyncClient(transport=ASGITransport(app=srv.app))`): 3/3 casos passaram — AC (sem ambiguidade, perfil registradora, 31 itens), CE (resolvida por "mais recente", perfil registradora, 31 itens), SE (a 310-2025 de suspensão, perfil financeira, 12 itens — **confirmado que a inscrição funciona normalmente mesmo sendo portaria de suspensão**, o backend não tem nenhuma checagem especial sobre o texto/tipo do ato, só `estado_sigla`+checklist do perfil). Idempotência do get-or-create de `POST /submissoes` também confirmada nos 3 casos (2ª chamada devolve a mesma submissão).

**Resultado final — link_pdf das 26 portarias vigentes escolhidas**: só **2 de 26 têm PDF anexado** (RN e SP — coincidentemente as 2 que passaram pelo dedup). As outras 24 (AC, AL, AM, AP, BA, CE, DF, ES, GO, MA, MG, MS, MT, PB, PE, PI, PR, RJ, RO, RR, RS, SC, SE, TO) estão com checklist certo mas **sem PDF** — ninguém consegue de fato se inscrever com base no ato real nessas UFs até o Pedro anexar o PDF manualmente via `Portarias.js` (Edit dialog). O checklist novo já funciona tecnicamente (testado), mas o PDF é o gargalo real pra inscrição de verdade na maioria das UFs.

**Deploy**: backend commit `5b9f02d` (auto-commit + push, escopo `backend/`, inclui só a migração — `server.py` não foi tocado neste item), rollback `sigcr-backend:pre-deploy-rollback-20260829-1206`. Sem mudança de frontend (nenhuma tela nova, só dados). Health check limpo (`/api/` 200, `/api/portarias`/`/api/checklist-catalogo` 401, sem erro nos logs). Estado final confirmado direto no Mongo de produção: 58 portarias ativas (60 − 2 duplicatas removidas), 26 com `checklist_itens` (27 UFs − PA), 27 UFs distintas ainda representadas (PA continua com suas 2 portarias antigas, só sem checklist).

**Não afetado, deliberado**: nenhuma portaria já publicada muda seu checklist retroativamente. Backfill de portarias reais pro catálogo novo (se algum dia fizer sentido) fica como decisão separada, não incluída aqui.

## 57. Upload real de PDF em Portarias (endpoint novo + Editar Portaria + wizard Criar Evento) — ✅ DEPLOYADO (2026-09-01)

Pedido do Pedro: item 56 deixou 24 de 26 portarias vigentes sem PDF (só RN/SP têm), gargalo real pra inscrição. Faltava um jeito de anexar o PDF direto pela UI em vez de só o campo de URL manual.

**Backend**: endpoint novo `POST /portarias/{portaria_id}/pdf` (multipart, campo `file`) — anexa/substitui o PDF de uma portaria já existente, salvando em `uploads/portarias/` (mesmo padrão de `/portarias/upload`). Permissão idêntica a Editar/Excluir: `require_perfil("sigcr_admin","detran","detran_admin")` + `_checar_permissao_escrita_estado` pela UF da portaria. Validação por **magic bytes** (assinatura `%PDF-`), não só Content-Type — o Content-Type de um multipart é declarado pelo cliente, trivialmente falsificável. Mesma checagem de magic bytes também adicionada ao endpoint já existente `/portarias/upload` (mesma classe de gap, mesmo arquivo, corrigido junto). Limite 20MB (`MAX_PORTARIA_PDF_SIZE`, já existia). **Sem ClamAV neste ambiente ainda — gap já sinalizado antes, este upload herda o mesmo risco residual, não o introduz.**

**Frontend**: `AreaTransparencia.js` — modal "Editar Portaria" ganhou um `<Input type="file">` ao lado do campo "Link do PDF" já existente; ao subir o arquivo, `link_pdf` é preenchido automaticamente com o path salvo (campo de URL manual continua editável por cima, pra quem preferir link externo). `CriarEvento.js` — passo "Revisar" do wizard ganhou o mesmo upload (opcional); se selecionado, o PDF é enviado logo após a portaria ser criada (antes de publicar, se for o caso), com fallback: falha no upload do PDF não derruba o fluxo, só avisa que ficou pendente pra anexar depois.

**Teste isolado — backend**: `httpx.AsyncClient(transport=ASGITransport(app=srv.app))` contra `sigcr-mongodb-devtest` (banco descartável `sigcr_devtest_uploadpdf`), candidato copiado pra `server_candidate.py` dentro do container (nunca sobrescreveu o `server.py` real rodando). 14/14 casos: detran da UF certa sobe PDF válido (200, `link_pdf` aponta pra `uploads/portarias/`, arquivo existe no disco com o conteúdo certo); detran de outra UF → 403; sigcr_admin → 200 em qualquer UF; registradora → 403 (bloqueado antes mesmo de chegar na função, via `require_perfil`); assinatura inválida com Content-Type falsificado → 400; Content-Type não-PDF → 400; arquivo > 20MB → 400; portaria inexistente → 404; sem sessão → 401; regressão confirmada em `/portarias/upload` (continua aceitando PDF válido, agora também rejeita assinatura inválida). PDFs de teste gerados pelo script foram removidos do disco depois (nenhum dado real tocado — as 2 portarias reais com PDF, RN/SP, ficaram intactas).

**Deploy**: backend commit `f4c63de`, rollback `sigcr-backend:pre-deploy-rollback-20260901-1421`. Frontend commit `ebd12dc`, release `releases/20260901-itemA-upload-pdf-portaria`. Health check: `/api/` 200, `/api/portarias` e a rota nova 401 sem token (existem, exigem auth), site 200 com hash novo.

## 58. Padrão único de clicáveis — Fase 3: `button.jsx` estendido (destructive-outline/destructive-ghost/tab) e propagado — ✅ DEPLOYADO (2026-09-01)

Continuação da proposta aprovada com mockup (Fase 1 auditoria + Fase 2 proposta, mesma sessão anterior): implementação em massa. Só frontend, nenhuma rota de API mudou.

**`ui/button.jsx`**: 3 variantes novas no `cva`, cobrindo os padrões achados na auditoria — `destructive-outline` (ação destrutiva secundária, ex: "Revogar"/"Rejeitar"), `destructive-ghost` (ação destrutiva inline sem moldura, ex: "Excluir" ao lado de "Editar"), e `tab` (pill de navegação, usa `data-[state=active]` — o mesmo atributo que o Radix `TabsTrigger` já expõe nativamente). A variante `destructive` já existia mas tinha **zero usos** em produção — agora é a base de toda ação destrutiva sólida.

**Unificação tabs/menubar**: `ui/tabs.jsx` (`TabsTrigger`, usado em Portarias\|Editais e nas 3 abas de `EstadoDetalhe.js`) e `components/ui/app-menu-bar.js` (Dashboard\|Transparência\|Notificações) agora consomem `buttonVariants({ variant: "tab" })` em vez de dois sistemas de cor próprios e divergentes (antes: um neutro shadcn puro, outro `bg-primary-500/15` ad hoc). `MenubarTrigger` (`menubar.jsx`) teve raio/padding/cor hardcoded removidos do próprio componente — passam a vir de `buttonVariants` via `className`, aplicado pelo caller. `MenubarTrigger` não expõe `data-[state=active]` de rota (seu `data-state` é só sobre o próprio submenu estar aberto), então `app-menu-bar.js` aplica a mesma cor via className condicionado ao booleano `isActive` já calculado ali — mesma cor final, wiring diferente, documentado em comentário nos dois arquivos.

**Migração dos ~8 tratamentos ad hoc de "Excluir"/ação destrutiva** achados na auditoria pra `destructive`/`destructive-outline`/`destructive-ghost`: `GestaoUsuarios.js`, `EmpresaFinanceira.js`, `EmpresaRegistradora.js`, `PainelConferencia.js`, `SolicitacaoDetalhe.js`, `FilaRegistros.js`, `AreaTransparencia.js`, `ChecklistContran.js` — incluindo a correção da altura órfã `h-7` (sem token correspondente) pra `size="sm"` (h-8) nos casos que eram texto, não ícone cravado.

**Limpeza da variante "outline"**: 13 usos que sobrescreviam a cor via `className="border-zinc-700 text-zinc-300..."` (reinventando o que a variante já define) tiveram o className redundante removido, em 8 arquivos.

**Conversão de `<button>` cru pra `<Button>`**: convertidos os que são genuinamente "botão de ação" (CTA, link de texto, ícone com espaço próprio) — `AreaTransparencia.js` (Ver PDF/Copiar link/Editar/Excluir/limpar/editar edital), `MinhasSubmissoes.js` e `PainelConferencia.js` ("← Voltar"), `ErrorBoundary.js` ("Voltar ao Dashboard" — era um botão azul reinventado do zero, idêntico ao variant default), `GestaoUsuarios.js` (toggle mostrar/ocultar senha), `Registradoras.js` ("limpar" filtro), `QueridoDiarioBusca.js` ("Filtros avançados"/"Limpar" resultados), `ChecklistContran.js` (os 3 itens do dropdown de ações do documento, incluindo "Excluir").

**Deliberadamente NÃO convertido** (julgamento, não esquecimento):
- **Cliques estruturais/de layout** (linha inteira de accordion/card expansível, card seletor de perfil) em `SolicitacaoDetalhe.js`, `Financeiras.js`, `Registradoras.js`, `GestaoUsuarios.js` (seletor de role) e o badge seletor de perfil em `DashboardLayout.js` — forçar esses no `Button` (inline-flex, rounded-md, altura fixa) quebraria layouts que dependem de `w-full`/altura variável/forma de badge (`rounded-full`); não são "botão de ação" no sentido que o Pedro pediu (Salvar/Excluir/Enviar/Buscar/tabs).
- **Ícones muito compactos** (`h-6 w-6`/`h-7 w-7`, ex: trigger do menu de documento em `ChecklistContran.js`, remover documento/anexo em `AreaTransparencia.js`) — não existe token de tamanho "ícone compacto" na escala atual do Button (`icon` = 36×36px, grande demais pra esses slots). Gap real, não resolvido aqui — precisaria de um novo `size` no `cva`, fora do escopo aprovado no mockup.
- **Chip de sugestão de busca** (`QueridoDiarioBusca.js`, pill `rounded-full` clicável) e o botão "Promover" (outline roxo/secondary) — formas/cores que não têm variante correspondente hoje (chip ≠ tab de navegação; não existe `secondary-outline`).
- **Todo um subsistema separado de páginas em inline `style={{}}` puro** (`AppMobile.js`, `Checkout.js`, `Planos.js`, `Esteiras.js`, `PagamentoAguardando.js`, `UploadDocumentos.js`, `ui/cookie-banner.js` — fluxo de assinatura/checkout Stripe + esteiras + cookie banner) — **zero Tailwind/shadcn**, cores em hex cru, sem `DashboardLayout`. Converter esses botões não é trocar `className` por `variant`, é reescrever o layout inteiro de cada tela pra Tailwind — escopo maior que o aprovado, e regressão em Checkout/Pagamento sem conseguir testar visualmente (sem navegador neste ambiente) é um risco real demais pra fazer como parte desta leva. **Recomendo tratar como item separado, com o Pedro confirmando o escopo antes.**

**Teste isolado**: build real (`craco build`) limpo, zero erro novo (warnings pré-existentes, nenhum nos arquivos alterados); compilado verificado via CSS/JS do bundle (`grep` confirmou `destructive-outline`/`destructive-ghost` presentes no JS, `data-[state=active]:bg-primary-500/15` presente no CSS compilado com a regra certa). Sem mudança de backend/contrato de API — nenhum teste de API necessário.

**Deploy**: frontend only. Commit `9a49d41`, release `releases/20260901-itemB-padrao-unico-botoes-tabs`. Site 200 com hash novo pós-swap.

**Ainda não confirmado por humano**: clique real no navegador confirmando que os pills de tab (Portarias\|Editais, Dashboard\|Transparência\|Notificações) ficaram visualmente idênticos entre si, e que nenhum botão convertido ficou com padding/altura estranha — sem navegador neste ambiente, a verificação foi por build+CSS compilado, não visual.

## 59. Botão "Credenciar-se" na Transparência, ponte direta pro Credenciamento por Portaria — ✅ DEPLOYADO (2026-09-01)

Pedido do Pedro: hoje `POST /submissoes` só é alcançável pela tela separada `MinhasSubmissoes.js` (`/credenciamento-portaria`), sem ponte visual desde a Transparência (`AreaTransparencia.js`) — registradora/financeira precisa saber que a outra tela existe.

**`AreaTransparencia.js`**: `podeCredenciar` — inverso exato de `podeGerenciar`, mesma lógica de `viewingAs` (sigcr_admin "vendo como" uma empresa precisa ver o botão; DETRAN/admin real, ou sigcr_admin sem simulação, nunca veem). Botão "Credenciar-se" novo em cada card de portaria, só quando `podeCredenciar`: chama `iniciarCredenciamento(portaria)` → `POST /submissoes` (mesmo endpoint/formato de chamada que `MinhasSubmissoes.js` já usa, get-or-create idempotente — clicar de novo numa portaria já iniciada só retoma) → navega pra `/credenciamento-portaria?submissao_id=X`, reaproveitando o deep-link que `MinhasSubmissoes.js` já lê (mesma infra das notificações, item 53) — **zero mudança de backend**. Portaria sem `link_pdf`: botão fica desabilitado com texto explicativo "PDF pendente de publicação" (não escondido, como pedido) em vez de simplesmente não aparecer.

**Item B aplicado direto**: o botão novo usa `<Button size="sm">` puro (variant default), sem nenhum className ad hoc — não criou mais um botão fora do padrão recém-definido.

**Teste isolado**: build real limpo (achado e corrigido no processo: um `)}` duplicado deixado por um edit anterior, pego pelo build antes do deploy). Lógica de `podeCredenciar` verificada por tabela-verdade contra os 5 casos reais (registradora/financeira reais, sigcr_admin simulando empresa, sigcr_admin simulando DETRAN, DETRAN/admin real sem simulação) — mutuamente exclusiva com `podeGerenciar` em todos os casos. Chamada a `POST /submissoes` é cópia literal do formato que `MinhasSubmissoes.js` já usa em produção (mesmo endpoint, mesmo shape de params) — não é uma chamada nova sem precedente.

**Deploy**: frontend only. Commit `f08c85c`, release `releases/20260901-itemC-credenciar-se-transparencia`. Site 200 com hash novo pós-swap.

**Ainda não confirmado por humano**: clique real de uma registradora/financeira na Transparência, incluindo o caso de portaria sem PDF (texto desabilitado) e o caso de sigcr_admin "vendo como" empresa vendo o botão aparecer — sem navegador neste ambiente, a verificação foi por build+leitura de código, não visual.

## 60. Modelo Credencia-CE — Fatia 1: catálogo `TipoCredenciamento` + `Company.categorias_credenciamento` (fundação, sem consumidor ainda) — ✅ DEPLOYADO (2026-09-01)

Primeira fatia de 5 do modelo aprovado por Pedro (investigação Fase 1, ver a discussão anterior desta sessão), inspirado no Credencia-CE (credencia.detran.ce.gov.br). Ordem confirmada: fundação → religação (checklist/submissão) → stepper de status → checklist accordion (base legal + responsabilidade) → filtro de categoria em Transparência. Esta fatia é deliberadamente só a fundação — aditiva, sem nenhum consumidor lendo os campos novos ainda.

**Modelo novo**: `TipoCredenciamento` (`tipo_id`, `nome`, `descricao`, `exige_vinculo_com: Optional[str]`, `ativo`) — catálogo de categorias de credenciamento. `exige_vinculo_com` modela como **dado** a regra que hoje é hardcoded em `_validar_tipo_e_vinculo_empresa` (financeira exige uma registradora com contrato ativo) — decisão B1: categorias novas nascem independentes por padrão, só quem setar esse campo explicitamente herda a exigência de vínculo. CRUD completo (`GET/POST/PATCH/DELETE /tipos-credenciamento`) — leitura só exige login (qualquer perfil), escrita é `require_perfil("sigcr_admin","detran","detran_admin")`, mesmo padrão do `checklist-catalogo`. Soft-disable nunca apaga, e bloqueia (409) se alguma empresa já usa a categoria ou se outra categoria ativa depende dela via `exige_vinculo_com` — dupla checagem de integridade referencial que o `checklist-catalogo` não precisa (não tem relação de dependência entre itens).

`Company` ganha `categorias_credenciamento: List[str] = []` — N:N por design (uma empresa pode ter várias categorias, decisão B2). `tipo_empresa` continua existindo e sendo o campo estrutural/legado (decide o vínculo financeira→registradora) — os dois eixos coexistem deliberadamente, `categorias_credenciamento` é só populado, ninguém ainda o lê pra decidir nada (isso é a Fatia 2).

**Migração** (`migrations/2026_09_01_tipo_credenciamento_fundacao.py`, idempotente): semeia os 2 registros do catálogo com `tipo_id` **fixo** (`"registradora"`/`"financeira"`, não gerado aleatoriamente) — é o que permite que todo dado já existente (`companies.tipo_empresa`, e futuramente `perfil_alvo` nos 4 modelos da Fatia 2) continue válido contra o catálogo sem precisar de nenhum rename. Backfill: toda empresa sem `categorias_credenciamento` preenchido recebe `[tipo_empresa]`; uma empresa com `tipo_empresa` fora do catálogo semeado receberia `[]` com aviso pra revisão manual (checado antes de rodar: produção só tem a HD Registros, `tipo_empresa="registradora"` — nenhum caso esquisito de verdade).

**Teste isolado — backend**: `httpx.AsyncClient(transport=ASGITransport(app=srv.app))` contra `sigcr-mongodb-devtest` (candidato como `server_candidate.py`, nunca sobrescreveu o `server.py` real rodando). 14/14 checks do CRUD: sem sessão → 401; qualquer perfil logado lê a lista → 200; só admin/detran/detran_admin cria/edita/desativa (registradora → 403); criar com `exige_vinculo_com` apontando pra tipo inexistente → 400; auto-referência (`exige_vinculo_com` == o próprio `tipo_id`) → 400; desativar um tipo do qual outro depende → 409; desativar um tipo em uso por uma empresa → 409; desativar um tipo livre (sem dependente, sem empresa usando) → 200. Banco descartável destruído depois.

**Teste isolado — migração**: rodada 3x contra um clone devtest com 5 empresas sintéticas cobrindo os casos de borda (`registradora`, `financeira`, uma que já tinha `categorias_credenciamento` preenchido — pulada corretamente —, uma com `tipo_empresa` fora do catálogo — sinalizada corretamente —, uma soft-deletada — excluída corretamente da contagem). `--dry-run` bateu exatamente com o plano real; 2ª rodada real confirmou idempotência (0 a inserir/preencher, exceto o caso esquisito de propósito, que a checagem por lista-vazia continua sinalizando a cada rodada — comportamento aceito deliberadamente: mantém o aviso visível até alguém corrigir o `tipo_empresa` daquela empresa, em vez de silenciar).

**Deploy**: backend commit `c6428a0` (auto-commit + push, escopo `backend/`), rollback `sigcr-backend:pre-deploy-rollback-20260901-1828`. Migração rodada em produção logo em seguida: `--dry-run` bateu exatamente com a checagem read-only feita antes (1 empresa real, HD Registros, `tipo_empresa=registradora`), real aplicou (catálogo semeado com os 2 tipos, HD Registros recebeu `categorias_credenciamento=["registradora"]`), 2ª rodada confirmou idempotência. Health check: `/api/` 200, `/api/tipos-credenciamento` 401 sem token (rota existe, exige auth). Sem mudança de frontend — nenhuma tela ainda lê o catálogo novo.
