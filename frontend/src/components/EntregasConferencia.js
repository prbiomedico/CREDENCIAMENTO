...
export default function EntregasConferencia({ ufs, onOpen }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled(key.split(',').map(uf => axios.get(`${API}/submissoes`, { params: { estado_sigla: uf }, withCredentials: true }))).then(results => {
      if (!active) return;
      setResult({ key, failed: results.some(r => r.status === 'rejected'), rows: results.flatMap(r => r.status === 'fulfilled' && Array.isArray(r.value.data) ? r.value.data : []).filter(s => (s.status === 'rascunho' && s.evidencias_para_conferencia) || s.status === 'submetido' || (s.status === 'em_analise' && s.itens?.some(i => i.status === 'enviado'))) });
    });
    return () => { active = false; };
  }, [key, version]);
  if (!key) return null;
  return <section className="rounded-lg border bg-card p-4 space-y-3" aria-label="Envios aguardando conferência">
    ...
    <p className="text-sm text-muted-foreground">Entregas finalizadas e evidências parciais identificadas no SIGCR. Selecione um pedido para conferir os documentos.</p>
    {!result || result.key !== key ? <p role="status">Consultando envios...</p> : <>
      ...
      {result.rows.map(s => <div key={s.submissao_id} className="flex flex-wrap justify-between items-center gap-3 rounded border p-3">
        <div><p className="font-medium">DETRAN-{s.estado_sigla} · {s.status === 'rascunho' ? 'Evidências parciais organizadas — pedido incompleto' : s.status === 'submetido' ? 'Novo envio para conferência' : 'Documentos aguardando análise'}</p><p className="text-sm text-muted-foreground">{s.itens?.filter(i => i.status === 'enviado').length || 0} itens aguardando conferência</p></div>
