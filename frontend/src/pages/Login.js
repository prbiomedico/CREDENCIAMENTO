    ...
    <main className="min-h-screen bg-slate-100 p-3 sm:p-6 lg:p-8">
      <div className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-[1440px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_80px_-32px_rgba(15,23,42,0.35)] sm:min-h-[calc(100vh-3rem)] lg:grid-cols-[minmax(0,1.08fr)_minmax(420px,0.92fr)]">
        <section className="relative hidden overflow-hidden bg-slate-950 px-12 py-10 text-white lg:flex lg:flex-col">
          ...
          <div className="absolute -right-24 top-24 h-80 w-80 rounded-full bg-primary-400/10 blur-3xl" />
          <div className="relative"><BrandLogo variant="horizontal" className="h-20 w-auto brightness-0 invert" alt="SIGCR — Sistema Integrado de Gestão de Credenciamento de Registradoras" /></div>
          <div className="relative my-auto max-w-2xl py-14">
            <p className="mb-5 text-sm font-semibold uppercase tracking-[0.18em] text-primary-300">Governança nacional</p>
            ...
            <div className="mt-12 grid max-w-xl grid-cols-3 gap-3">{[['DETRANs', Landmark], ['Registradoras', Building2], ['Acesso auditável', BadgeCheck]].map(([label, Icon]) => <div key={label} className="rounded-xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur-sm"><Icon className="mb-7 h-5 w-5 text-primary-300" /><p className="text-sm font-medium text-slate-200">{label}</p></div>)}</div>
          </div>
        ...
        <section className="flex items-center justify-center px-6 py-10 sm:px-12 lg:px-16">
          <div className="w-full max-w-md">
            <BrandLogo variant="horizontal" className="mb-12 h-16 w-auto lg:hidden" alt="SIGCR" />
            <div className="mb-8"><span className="mb-6 grid h-12 w-12 place-items-center rounded-xl border border-primary-200 bg-primary-50"><LockKeyhole className="h-5 w-5 text-primary-600" /></span><p className="text-sm font-semibold text-primary-600">Acesso seguro</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.035em] text-slate-950">Entre no ambiente SIGCR</h2><p className="mt-3 text-base leading-7 text-slate-600">Use sua identidade institucional. Você será direcionado ao provedor seguro para concluir a autenticação.</p></div>
